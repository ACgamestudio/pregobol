...
const Especiais = {
  ...
  armar(n, tipo, remoto) {
    ...
    /* Replay of the opponent's flick: they already paid for it on their
       own device, so the local ledger must not veto it. */
    if (remoto) { this.armado[n] = tipo; this.cargas[n] = Math.max(this.cargas[n], e.custo); return 'armado'; }
