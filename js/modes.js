...
const Modo = {
  ...
  /* ---------------- entry points ---------------- */
  /* Online. Progression deliberately does NOT advance: unlocks are a
     single-player reward and a friend's device should not grant them. */
  online(cfg) {
    this.tipo = 'online';
    this.etapa = 0; this.desafio = null;
    modo = 'online';
    return { alvo: (cfg && cfg.alvo) || 3 };
  },
  ...
  rotulo() {
    ...
    if (this.tipo === 'online') return t('online') + (Rede.sala ? ' · ' + Rede.sala : '');
    return '';
  },
  ...
  fimDaPartida(vencedor) {
    if (this.tipo === 'online') {
      const ganhei = vencedor === Rede.sou;
      return { titulo: ganhei ? t('youWin') : t('youLose'),
               sub: t('online') + ' · ' + (Rede.sala || ''),
               botao: t('menu'), acao: 'menu' };
    }
