    ...
    for (const pr of pregos) {
      ...
      if (!silencioso) {
        ...
        if (T.bonus && !pr.tocado) {
          ...
          if (!silencioso) avisarCarga(e.dono || 1, 2);
          Modo.contarDourado();
...
function reagirAoQuase() {
  ...
  /* Quase-gol paga uma carga. Sem isso, as cargas só vinham de gol com
     combo e de prego dourado: medido, dava 1,5 por jogador numa partida
     inteira, enquanto o especial mais barato custa 2. Muita gente
     terminaria o jogo sem nunca poder usar um. Raspar a trave é
     habilidade e merece pagar. */
  Especiais.ganhar(jogador, 1);
  avisarCarga(jogador, 1);
  atualizarEspeciais();
}
...
/* Ganhar carga precisa ser VISÍVEL. Elas só vêm de prego dourado e de
   gol com combo, o que já é raro — se ainda por cima entram em silêncio,
   o jogador nunca descobre que o sistema existe. */
function avisarCarga(quem, k) {
  if (!k || quem !== jogador) return;
  FX.grito(`+${k} ⚡`, t('chargeEarned'), '#FFD24A');
  const cx = document.querySelector('.cargasCaixa');
  if (cx) { cx.classList.remove('ganhou'); void cx.offsetWidth; cx.classList.add('ganhou'); }
}
