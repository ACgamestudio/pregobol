...
const FX = (() => {
  ...
  function golExplosao(x, y, cores) {
    solta(x, y, 46, cores[0], 3.4);
    solta(x, y, 30, cores[1] || '#FFD24A', 2.6);
    solta(x, y, 18, '#FFF6DC', 4.2, 'brilho');
    sacudir(11);
    flash = 1;
  }
  ...
  function atualizar(passos) {
    ...
    if (flash > 0) flash = Math.max(0, flash - 0.045 * k);
  ...
  function limpar() { particulas = []; gritos = []; sacudida = 0; flash = 0; lento = 0; cam = null; }

  return { solta, leque, impacto, golExplosao, sacudir, devagar, focar, grito,
