...
en: {
  ...
  worldTeams: 'WORLD TEAMS', brTeams: 'BRAZIL TEAMS',
  introCard: 'INTRO', introOn: 'on', introOff: 'skip to menu',
  chargeEarned: 'special charge',
  howCharges: 'CHARGES: score off a bank shot, a nail or a double rebound (+1 to +2). A golden nail gives +2 on contact.',
  ...
  opponentTurn: "opponent's turn", yourTurn: 'YOUR TURN',
  opponentLeft: 'the opponent left the match', connecting: 'connecting…',
  netFail: 'could not connect', roomGone: 'room not found or already full',
  roomMissing: 'this room does not exist (closed, or wrong code)', roomFull: 'this room already has two players',
  hostAway: 'the room owner needs to have the game open', sendWhatsApp: '📲 SEND ON WHATSAPP',
  invited: 'you were invited! tap JOIN', resumed: 'your room is still open — waiting for your friend',
  comeBack: 'sent? come back here and wait',
  joinedWaitHost: s => `joined ✓ — waiting for the room owner to come back to the game${s ? ` (last seen ${s}s ago)` : ''}`,
  friendJoined: 'your friend joined ✓ — connecting…',
  friendJoinedWait: 'your friend joined ✓ — waiting for their phone to connect…',
  selfReconnecting: 'your friend joined ✓ — reconnecting you…',
  joinedWaiting: 'you are in! waiting for the room owner to come back to the game…',
  inviteMsg: c => `Let's play Pregobol! Tap the link and hit JOIN (room ${c}):`,
  holdToTalk: 'hold 🎙️ to talk', micDenied: 'microphone blocked — allow it in the browser',
  micUnsupported: 'this browser cannot record audio', voiceSent: 'voice message sent',
  voiceFail: 'voice message not sent', voiceFrom: '🎙️ opponent',
  shareCode: 'send this code to your friend', soon: 'coming soon',
  on: 'On', off: 'Off',
...
pt: {
  ...
  worldTeams: 'TIMES DO MUNDO', brTeams: 'TIMES DO BRASIL',
  introCard: 'ABERTURA', introOn: 'ligada', introOff: 'ir direto ao menu',
  chargeEarned: 'carga especial',
  howCharges: 'CARGAS: faça gol de tabela, de prego ou com dois rebotes (+1 a +2). Prego dourado dá +2 só de encostar.',
  ...
  opponentTurn: 'vez do adversário', yourTurn: 'SUA VEZ',
  opponentLeft: 'o adversário saiu da partida', connecting: 'conectando…',
  netFail: 'não foi possível conectar', roomGone: 'sala não encontrada ou já cheia',
  roomMissing: 'essa sala não existe (fechou ou o código está errado)', roomFull: 'essa sala já tem dois jogadores',
  hostAway: 'quem criou a sala precisa estar com o jogo aberto', sendWhatsApp: '📲 MANDAR NO WHATSAPP',
  invited: 'você foi convidado! aperte ENTRAR', resumed: 'sua sala continua aberta — esperando seu amigo',
  comeBack: 'mandou? volte pra cá e espere',
  joinedWaitHost: s => `entrou ✓ — esperando o dono da sala voltar pro jogo${s ? ` (visto há ${s}s)` : ''}`,
  friendJoined: 'seu amigo entrou ✓ — conectando…',
  friendJoinedWait: 'seu amigo entrou ✓ — esperando o celular dele conectar…',
  selfReconnecting: 'seu amigo entrou ✓ — reconectando você…',
  joinedWaiting: 'você entrou! esperando o dono da sala voltar pro jogo…',
  inviteMsg: c => `Bora jogar Pregobol! Toca no link e aperta ENTRAR (sala ${c}):`,
  holdToTalk: 'segure 🎙️ pra falar', micDenied: 'microfone bloqueado — libere no navegador',
  micUnsupported: 'este navegador não grava áudio', voiceSent: 'mensagem de voz enviada',
  voiceFail: 'mensagem de voz não enviada', voiceFrom: '🎙️ adversário',
  shareCode: 'mande este código pro seu amigo', soon: 'em breve',
...
let idioma = 'pt';   // default; the IDIOMA card in the menu flips it
...
function aplicarIdioma() {
  ...
  setHTML('cardOnline', `<span class="ico">🌐</span>${t('online')}<em>${t('onlineSub')}</em>`);
  ...
  const bf = document.getElementById('btnFaixa');
  if (bf) bf.title = t('track');
  ...
  set('tituloOnline', t('online'));
  set('rotCodigo', t('roomCode'));
  set('dicaCodigo', t('shareCode'));
  set('btnCriarSala', t('createRoom'));
  set('btnProcurar', t('findMatch'));
  set('btnEntrarSala', t('joinRoom'));
  set('voltarMenu5', t('back'));
