...
const Som = (() => {
  ...
  /* ---------------------------------------------------------------
     Recorded samples. Three voices are real recordings now; everything
     else stays synthesised. Each sample falls back to its synthesised
     version if the file is missing or the browser refuses to decode it,
     so the game never ends up silent because of a bad path.
     --------------------------------------------------------------- */
  const AMOSTRAS = {
    gol: 'assets/gol.mp3',
    apitoInicio: 'assets/apito_inicio.mp3',
    apitoFinal: 'assets/apito_final.mp3',
    torcida: 'assets/torcida.mp3'
  };
  const banco = {};
  Object.keys(AMOSTRAS).forEach(nome => {
    const el = new Audio();
    const reg = { el: el, ok: false };
    el.preload = 'auto';
    el.addEventListener('canplaythrough', () => { reg.ok = true; }, { once: true });
    el.addEventListener('error', () => { reg.ok = false; });
    el.src = AMOSTRAS[nome];
    banco[nome] = reg;
  });

  /* Cloned per play so two goals in quick succession overlap instead of
     cutting each other off. */
  function amostra(nome, vol) {
    if (!ligado) return false;
    const reg = banco[nome];
    if (!reg || !reg.ok) return false;
    try {
      const s = reg.el.cloneNode();
      s.volume = vol === undefined ? 1 : vol;
      const p = s.play();
  ...
  function inicio() {
    if (amostra('apitoInicio', .85)) return;
    if (!ligado) return; const a = ligar(), t = a.currentTime;
    apito(false);
  ...
  /* The crowd reacting to a near miss. No synthesised fallback: an "oooh"
     is a recording or it is nothing, and a fake one would sound worse
     than silence. */
  function torcida() { amostra('torcida', .8); }
  /* Full time. */
  function apitoFinal() {
    if (amostra('apitoFinal', .85)) return;
    apito(true);
  }
  ...
  return { peteleco, prego, pregoMola, pregoDourado, madeira, tampa, gol, apito,
           apitoFinal, torcida, fanfarra, derrota, botao, inicio, especial, buraco, ligar,
