...
function acomodarPregos(k) {
  for (const pr of pregos) {
    if (pr.mov) {
      pr.y = pr.baseY + Math.sin(Date.now() * pr.mov.vel + pr.mov.fase) * pr.mov.amp * CAMPO.h * 0.5;
...
function passoFisico(e, silencioso) {
  ...
  for (let s = 0; s < sub; s++) {
    ...
    if (ESCORREGA) {
      e.vx += (Math.random() - .5) * ESCORREGA;
      e.vy += (Math.random() - .5) * ESCORREGA;
    }
    ...
    for (const pr of pregos) {
      ...
      if (T.desvio) {
        ...
        const a = Math.atan2(rvy, rvx) + (Math.random() - .5) * 2 * T.desvio;
    ...
    if (e.x + rb > CAMPO.x + CAMPO.w) {                 // right goal
      ...
      else { e.x = CAMPO.x + CAMPO.w - rb; e.vx = -e.vx * restP; bateuNaRipa(e, silencioso, vel, -1, 0); }
    }
    if (e.x - rb < CAMPO.x) {                           // left goal
      if (naBoca) { if (e.x < CAMPO.x - 1) return 2; }
      else { e.x = CAMPO.x + rb; e.vx = -e.vx * restP; bateuNaRipa(e, silencioso, vel, 1, 0); }
    }
    if (e.y - rb < CAMPO.y) {
...
function passo() {
  ...
  if (lenta || demorou) { bola.vx = bola.vy = 0; bola.giro = 0; fimDoToque(); }
}
...
function fimDeJogo(vencedor) {
  ...
  if (humano) { Som.fanfarra(); celebrar(vencedor); } else { Som.derrota(); }
